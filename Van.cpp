...
CVan::CVan(LPDIRECT3DDEVICE9 pGraphicDev)
	: CGameObject(pGraphicDev)
{

}

CVan::CVan(const CVan& rhs)
	: CGameObject(rhs)
{
...
HRESULT CVan::Ready_Object(void)
{
...
Engine::_int CVan::Update_Object(const _float& fTimeDelta)
{
...
HRESULT CVan::Add_Component(void)
{
	CComponent*			pComponent = nullptr;

	// StaticMesh
	pComponent = m_pMeshCom = dynamic_cast<CStaticMesh*>(Clone_Proto(L"Proto_Mesh_Van"));
...
void CVan::SetUp_OnTerrain(void)
{
	_vec3	vPos;
	m_pTransformCom->Get_Info(INFO_POS, &vPos);
...
Engine::_bool CVan::Collision_ToObject(const _tchar* pLayerTag, const _tchar* pObjTag)
{
...
HRESULT CVan::SetUp_ConstantTable(LPD3DXEFFECT& pEffect)
{
...
CVan* CVan::Create(LPDIRECT3DDEVICE9 pGraphicDev)
{
	CVan*	pInstance = new CVan(pGraphicDev);
