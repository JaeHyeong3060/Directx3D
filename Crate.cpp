...
CCrate::CCrate(LPDIRECT3DDEVICE9 pGraphicDev)
...
HRESULT CCrate::Ready_Object(void)
{
...
Engine::_int CCrate::Update_Object(const _float& fTimeDelta)
{
...
void CCrate::Render_Object(void)
{
	//if (false == m_bDraw)
	//	return;
...
HRESULT CCrate::Add_Component(void)
{
	CComponent*			pComponent = nullptr;

	// DynamicMesh
	pComponent = m_pMeshCom = dynamic_cast<CDynamicMesh*>(Clone_Proto(L"Proto_Mesh_Crate"));
...
void CCrate::SetUp_OnTerrain(void)
{
	_vec3	vPos;
	m_pTransformCom->Get_Info(INFO_POS, &vPos);
...
Engine::_bool CCrate::Collision_ToObject(const _tchar* pLayerTag, const _tchar* pObjTag)
{
...
HRESULT CCrate::SetUp_ConstantTable(LPD3DXEFFECT& pEffect)
{
...
CCrate* CCrate::Create(LPDIRECT3DDEVICE9 pGraphicDev)
{
	CCrate*	pInstance = new CCrate(pGraphicDev);
