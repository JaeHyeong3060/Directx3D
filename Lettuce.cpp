...
/* mesh
	LPSTR test = "Lettuce_Whole";
	LPSTR test = "Lettuce_Sliced";
*/
...
CLettuce::CLettuce(const CLettuce& rhs)
	: CGameObject(rhs)
{

}

CLettuce::~CLettuce(void)
{

}

HRESULT CLettuce::Ready_Object(void)
{
	...
	m_wstrObjTag = L"Lettuce";

	return S_OK;
}

Engine::_int CLettuce::Update_Object(const _float& fTimeDelta)
{
...
void CLettuce::Render_Object(void)
{
	//if (false == m_bDraw)
	//	return;
...
HRESULT CLettuce::Add_Component(void)
{
	CComponent*			pComponent = nullptr;

	// DynamicMesh
	//pComponent = m_pMeshCom = dynamic_cast<CDynamicMesh*>(Clone_Proto(L"Proto_Mesh_Lettuce"));
...
void CLettuce::SetUp_OnTerrain(void)
{
	_vec3	vPos;
	m_pTransformCom->Get_Info(INFO_POS, &vPos);
...
Engine::_bool CLettuce::Collision_ToObject(const _tchar* pLayerTag, const _tchar* pObjTag)
{
...
HRESULT CLettuce::Set_LettuceIndex(int iIndex)
{
	CComponent* pComponent = nullptr;
	...
	if (iIndex == 1)
	{
		pComponent = m_pMeshCom = dynamic_cast<CDynamicMesh*>(Clone_Proto(L"Proto_Mesh_LettuceB"));	
	}
	if (iIndex == 2)
	{
		pComponent = m_pMeshCom = dynamic_cast<CDynamicMesh*>(Clone_Proto(L"Proto_Mesh_LettuceC"));
	}
	if (iIndex == 3)
	{
		pComponent = m_pMeshCom = dynamic_cast<CDynamicMesh*>(Clone_Proto(L"Proto_Mesh_LettuceD"));
	}
	if (iIndex == 4)
	{
		pComponent = m_pMeshCom = dynamic_cast<CDynamicMesh*>(Clone_Proto(L"Proto_Mesh_LettuceE"));
	}
	if (iIndex == 5)
	{
		pComponent = m_pMeshCom = dynamic_cast<CDynamicMesh*>(Clone_Proto(L"Proto_Mesh_LettuceF"));
	}
	NULL_CHECK_RETURN(pComponent, E_FAIL);
	m_mapComponent[ID_STATIC].emplace(L"Com_Mesh", pComponent);
...
HRESULT CLettuce::SetUp_ConstantTable(LPD3DXEFFECT& pEffect)
{
...
{
	CLettuce*	pInstance = new CLettuce(pGraphicDev);
