...
HRESULT CMyMap::Ready_Object(void)
{
...
Engine::_int CMyMap::Update_Object(const _float& fTimeDelta)
{
...
void CMyMap::Render_Object(void)
{
	/*if (false == m_bDraw)
		return;*/
...
HRESULT CMyMap::Add_Component(void)
{
	CComponent*			pComponent = nullptr;

	// StaticMesh
	pComponent = m_pMeshCom = dynamic_cast<CStaticMesh*>(Clone_Proto(L"Proto_Mesh_MyMap"));
...
void CMyMap::SetUp_OnTerrain(void)
{
	_vec3	vPos;
	m_pTransformCom->Get_Info(INFO_POS, &vPos);
	...
	_float		fHeight = m_pCalculatorCom->Compute_HeightOnTerrain(&vPos, pTerrainBufferCom->Get_VtxPos(), VTXCNTX, VTXCNTZ)-0.1f;

	m_pTransformCom->Set_Pos(vPos.x, fHeight, vPos.z);
}

Engine::_bool CMyMap::Collision_ToObject(const _tchar* pLayerTag, const _tchar* pObjTag)
{
...
HRESULT CMyMap::SetUp_ConstantTable(LPD3DXEFFECT& pEffect)
{
