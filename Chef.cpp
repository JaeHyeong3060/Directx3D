...
CChef::CChef(LPDIRECT3DDEVICE9 pGraphicDev)
...
HRESULT CChef::Ready_Object(void)
{
...
Engine::_int CChef::Update_Object(const _float& fTimeDelta)
{
	...
	if (m_bCookState)
	{
		CSoundMgr::Get_Instance()->PlaySound(L"KnifeChopCut.wav", CSoundMgr::PLAYER_A);
	}
	if (m_bWashState)
	{
		CSoundMgr::Get_Instance()->PlaySound(L"Washing_01.wav", CSoundMgr::PLAYER_A);
	}
...
HRESULT CChef::Add_Component(void)
{
	CComponent* pComponent = nullptr;

	// DynamicMesh
	pComponent = m_pMeshCom = dynamic_cast<CDynamicMesh*>(Clone_Proto(L"Proto_Mesh_Chef"));
...
void CChef::Key_Input(const _float& fTimeDelta)
{
	...
	if (Get_DIKeyState(DIK_UP) & 0x80)
	{
		...
		CSoundMgr::Get_Instance()->PlaySound(L"Step_01.wav",CSoundMgr::PLAYER_A_ENV);
	}
...
#pragma region EVENT_KEY_Z
	if(CKeyManager::GetInstance()->Key_Up(KEY_Z))
	{
		if (m_bCookState == true)
			RemoveKnife();
		m_bCookState = false;
...
#pragma region Chef_Inventory_EMPTY
		if (m_listInven.empty())
		{
			// 인벤X Crate 양파 충돌
			if (m_dwColState & COL_CRATE_ONION)
			{
				...
				static_cast<COnion*>(pGameObject)->Set_OnionIndex((++m_OnionIndex)%3);
				...
				CSoundMgr::Get_Instance()->PlaySound(L"AddIngredient_01.wav", CSoundMgr::PLAYER_A);
			}
			// 인벤X Crate 토마토 충돌
			if (m_dwColState & COL_CRATE_TOMATO)
			{
				...
				static_cast<CTomato*>(pGameObject)->Set_TomatoIndex((++m_TomatoIndex) % 3);
			...
			if (m_dwColState & COL_CRATE_LETTUCE)
			{
				...
				static_cast<CLettuce*>(pGameObject)->Set_LettuceIndex((++m_TomatoIndex) % 3);
				...
				if (m_DestObj->Get_Inven()->size() != 0)
				{
					...
					if (pfrontItemInven->Get_ObjTag() == L"Onion")
					{
						...
						if (bIsFinished)
						{
							...
							CSoundMgr::Get_Instance()->PlaySound(L"Item_PickUp_01.wav", CSoundMgr::PLAYER_A);
						}
					}
		...
		else if (!m_listInven.empty())
		{
			...
			if (m_dwColState & COL_THEPASS)
			{
				if (m_listInven.front()->Get_ObjTag() == L"Plate")
				{
					...
					CSoundMgr::Get_Instance()->PlaySound(L"SuccessfulDelivery.wav", CSoundMgr::PLAYER_A);
				}
			...
			if (m_dwColState & COL_CHOPPINGBOARD)
			{
				...
				CSoundMgr::Get_Instance()->PlaySound(L"Item_PutDown_01.wav", CSoundMgr::PLAYER_A);
			}
			// 아이템 들고 빈 공간에 
			if (m_dwColState & COL_EMPTY)
			{
...
#pragma region EVENT_KEY_X
	if (CKeyManager::GetInstance()->Key_Up(KEY_X))
	{
		// 인벤 X
		if (m_listInven.empty())
		{
...
#pragma endregion
	...
	//if (CKeyManager::GetInstance()->Key_Up(KEY_P))
	//{
	//	CUIManager::GetInstance()->Add_Menu(L"SoupOnion");	
	//}
	//if (CKeyManager::GetInstance()->Key_Up(KEY_T))
	//{
	//	CUIManager::GetInstance()->Add_Menu(L"Salad");	
	//}
	//if (CKeyManager::GetInstance()->Key_Up(KEY_Y))
	//{
	//	CUIManager::GetInstance()->Add_Menu(L"SoupTomato");
	//}

	if (m_bCookState && m_pMeshCom->Is_AnimationsetFinish())
	{
		m_pMeshCom->Set_AnimationIndex(0);
		m_pMeshCom->Set_AnimationIndex(1);
	}
...
void CChef::SetUp_OnTerrain(void)
{
	_vec3	vPos;
	m_pTransformCom->Get_Info(INFO_POS, &vPos);
...
Engine::_vec3 CChef::PickUp_OnTerrain(void)
{
	CTerrainTex* pTerrainBufferCom = dynamic_cast<CTerrainTex*>(Engine::Get_Component(L"GameLogic", L"Terrain", L"Com_Buffer", ID_STATIC));
...
HRESULT CChef::SetUp_ConstantTable(LPD3DXEFFECT& pEffect)
{
...
Engine::_bool CChef::Collision_ToObject(const _tchar* pLayerTag, const _tchar* pObjTag)
{
	auto CurScene = CManagement::GetInstance()->Get_Scene();
...
_bool CChef::Body_Collision_ToObject(const _tchar* pLayerTag, const _tchar* pObjTag)
{
	auto CurScene = CManagement::GetInstance()->Get_Scene();
	auto CurLayer = CurScene->Get_Layer(pLayerTag);
...
void CChef::PUSH_KEY_Z_COL_EMPTY()
{
	auto pGameObject = m_listInven.front();
	auto transform = dynamic_cast<CTransform*>(pGameObject->Get_Component(L"Com_Transform", ID_DYNAMIC));
	auto vDestPos = m_DestCol->Get_CollWorldMatrix();
...
void CChef::RemoveKnife()
{
	LPSTR meshStr = "Knife";
	m_pMeshCom->Add_RemoveStr(&meshStr);
}

void CChef::CreateEffectStar()
{
	...
	static_cast<CEffect*>(pGameObject)->Set_TextureWstr(L"Effect_Star");
	auto transform = dynamic_cast<CTransform*>(pGameObject->Get_Component(L"Com_Transform", ID_DYNAMIC));
...
CChef* CChef::Create(LPDIRECT3DDEVICE9 pGraphicDev)
{
	CChef* pInstance = new CChef(pGraphicDev);
