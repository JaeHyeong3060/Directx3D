...
CFireExtinguisher::CFireExtinguisher(const CFireExtinguisher& rhs)
	: CGameObject(rhs)
{

}

CFireExtinguisher::~CFireExtinguisher(void)
{

}

HRESULT CFireExtinguisher::Ready_Object(void)
{
...
Engine::_int CFireExtinguisher::Update_Object(const _float& fTimeDelta)
{
...
void CFireExtinguisher::Render_Object(void)
{
	if (false == m_bDraw)
		return;
...
HRESULT CFireExtinguisher::Add_Component(void)
{
	CComponent*			pComponent = nullptr;

	// StaticMesh
	pComponent = m_pMeshCom = dynamic_cast<CStaticMesh*>(Clone_Proto(L"Proto_Mesh_FireExtinguisher"));
...
void CFireExtinguisher::SetUp_OnTerrain(void)
{
	_vec3	vPos;
	m_pTransformCom->Get_Info(INFO_POS, &vPos);
...
Engine::_bool CFireExtinguisher::Collision_ToObject(const _tchar* pLayerTag, const _tchar* pObjTag)
{
...
HRESULT CFireExtinguisher::SetUp_ConstantTable(LPD3DXEFFECT& pEffect)
{
...
{
	CFireExtinguisher*	pInstance = new CFireExtinguisher(pGraphicDev);
...
void CFireExtinguisher::Free(void)
{
	CGameObject::Free();
}
